define("./workbox-7a2a8380.js", ["exports"], (function (t) {
	...
	class o {
		...
		findMatchingRoute({
			...
			for (const r of i) {
				...
				if (o) return i = o, (Array.isArray(i) && 0 === i.length || o.constructor === Object && 0 === Object.keys(o)
					.length || "boolean" == typeof o) && (i = void 0), {
					route: r,
					params: i
				}
			}
		...
		registerRoute(t) {
			this.t.has(t.method) || this.t.set(t.method, []), this.t.get(t.method)
				.push(t)
		}
		unregisterRoute(t) {
	...
	const c = {
		...
		h = t => [c.prefix, t, c.suffix].filter((t => t && t.length > 0))
			.join("-"),
		u = t => t || h(c.precache),
		l = t => t || h(c.runtime);
	...
	class U {
		...
		async cachePut(t, e) {
			...
			if (!e) throw new s("cache-put-with-no-response", {
				url: (o = r.url, new URL(String(o), location.href)
		...
		async getCacheKey(t, e) {
			if (!this.u[e]) {
				let s = t;
				for (const t of this.iterateCallbacks("cacheKeyWillBeUsed")) s = q(await t({
					mode: e,
					request: s,
					event: this.event,
					params: this.params
	...
	class b extends class {
		...
		constructor(t = {}) {
			t.cacheName = u(t.cacheName), super(t), this._ = !1 !== t.fallbackToNetwork, this.plugins.push(b.copyRedirectedCacheableResponsesPlugin)
		}
		async L(t, e) {
		...
		N() {
			let t = null,
				e = 0;
			for (const [s, n] of this.plugins.entries()) n !== b.copyRedirectedCacheableResponsesPlugin && (n === b.defaultPrecacheCacheabilityPlugin && (t = s), n.cacheWillUpdate && e++);
	...
	class L {
		...
		addToCacheList(t) {
			...
			for (const n of t) {
				...
				if ("string" != typeof n && n.integrity) {
					if (this.K.has(t) && this.K.get(t) !== n.integrity) throw new s("add-to-cache-list-conflicting-integrities", {
						url: i
					});
		...
		async matchPrecache(t) {
			...
			if (s) {
				return (await self.caches.open(this.strategy.cacheName))
					.match(s)
			}
		}
